...
fn main() {
    ...
    let output_buffer: RgbImage = ImageBuffer::new(image_size.x.try_into().unwrap(), image_size.y.try_into().unwrap());
    
    render(image_size, constant, 200, output_buffer, grayscale);
    // render(image_size, constant, 200, output_buffer, colorgrad::magma());
}
...
fn compute_next_julia(current: Vec2, constant: Vec2) -> Vec2 {
    ...
    Vec2 {x: z_real + constant.x, y: z_imaginary + constant.y}
}
...
fn render(render_size: IVec2, constant: Vec2, max_iterations: usize, mut image: RgbImage, gradient: Gradient) {
    ...
    while y < render_size.y {
        ...
        while x < render_size.x {
            ...
            let iterations = iterate_to_max_julia(Vec2 { x: pixel_x, y: pixel_y }, constant, max_iterations);

            let normalised_value = iterations as f64 / max_iterations as f64;
            let colour = gradient.at(normalised_value).to_rgba8();

            image.put_pixel(x.try_into().unwrap(), y.try_into().unwrap(), Rgb([colour[0], colour[1], colour[2]]));
